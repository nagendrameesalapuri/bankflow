 ...
 *
 * Lives under backend/src (rather than database/seeds, where the project's
 * documented structure originally placed it) so it compiles into
 * backend/dist as plain JS - production has no TypeScript loader, so a
 * runtime require() of a raw .ts file would fail there. database/seeds/seed.ts
 * still exists as a thin re-export wrapper for local dev via tsx.
 */
